...
public class Servico extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        String nomeClasse = "com.stolk.alecsandro.contas.controle.contato." + parametroAcao;
        String nome;
        try {
        ...
        if (nomes[0].equalsIgnoreCase("forward")) {
            RequestDispatcher rd = request.getRequestDispatcher("WEB-INF/view/contato/" + nomes[1]);
            rd.forward(request, response);
        } else {
            response.sendRedirect("contatos?acao=" + nomes[1]);
        }
    }
}
